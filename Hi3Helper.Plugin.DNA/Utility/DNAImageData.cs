...
internal partial class DNAImageDataMap : JsonSerializerContext;
#endif

public static partial class DNAImageData
{
    private static Dictionary<string, string>? _dnaIconDataMapDictionary;
    ...
    private static async Task LoadEmbeddedData(CancellationToken token)
    {
        ...
        while (await tarReader.GetNextEntryAsync(true, token) is { } entry)
        {
            ...
            {
                _dnaIconDataMapDictionary =
#if USELIGHTWEIGHTJSONPARSER
                    await GetDictionaryAsync(copyToStream, token);
#else
                    await JsonSerializer.DeserializeAsync(copyToStream, DNAImageDataMap.Default.DictionaryStringString, token);
#endif
                ...
                _dnaIconDataMapDictionary.Clear();
                foreach (KeyValuePair<string, string> a in keyValueReversed)
                {
                    _dnaIconDataMapDictionary.Add(a.Value, a.Key);
                }
